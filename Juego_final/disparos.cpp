...
{
    if ( E.n_disp > 0 && E.n_disp < E.max_disp){
            for ( int cont = 1; cont <= E.n_disp; cont++)
            {
                if(colision(N.x,N.y,N.ancho_p,N.alto_p,B[cont].x,B[cont].y,E.ancho_b,E.alto_b)&& N.vida>0)
                {
                    eliminar(B,E.n_disp,cont);
                    N.vida--;
                    return true;
                }
            }
